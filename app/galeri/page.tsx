...
export default function GaleriPage() {
  ...
  useEffect(() => {
    ...
    if (filterRef.current) {
      registerElement('filter', filterRef.current);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section 
        ref={headerRef}
        className={`bg-primary text-primary-foreground py-12 transition-all duration-700 ease-out transform-gpu ${
      ...
      <section className="py-12 md:py-16 flex-1">
        <div className="container mx-auto px-4">
          ...
          <div
            ref={filterRef}
            className={`flex flex-wrap gap-3 mb-8 justify-center transition-all duration-700 ease-out transform-gpu ${
              isVisible['filter'] !== undefined
                ? (isVisible['filter']
                    ? 'opacity-100 translate-y-0'
                    : 'opacity-0 translate-y-10')
                : 'opacity-100 translate-y-0' // Default to visible if not yet observed
          ...
          >
            <button
              ...
              className={`px-6 py-2 rounded-full font-medium transition-all duration-700 hover-scale ${
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.map((item, index) => (
              <Card 
                key={item.id} 
                className={`overflow-hidden hover:shadow-lg transition-all duration-700 flex flex-col h-full animate-fade-in-up cursor-pointer`}
